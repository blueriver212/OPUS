...
Usage:
    python plot_welfare_umpy_from_csv.py
...
def create_plot_from_csv(csv_path, output_path=None):
    ...
    # Colors
    satellites_color_5yr = "blue"
    satellites_color_25yr = "darkblue"
    umpy_color_5yr = "red"
    umpy_color_25yr = "darkred"
    
    # Plot Welfare % change (5yr - solid)
    if not df_5yr.empty and df_5yr['satellites_percent_change'].notna().any():
        ...
        ax.plot(
            ...
            color=satellites_color_5yr, 
            marker='s', 
            linestyle='-', 
            linewidth=2, 
            markersize=6, 
            label='Welfare % Change (5yr)', 
            zorder=3
        )
    
    # Plot Welfare % change (25yr - dashed)
    if not df_25yr.empty and df_25yr['satellites_percent_change'].notna().any():
        ...
        ax.plot(
            ...
            color=satellites_color_25yr, 
            marker='s', 
            linestyle='--', 
            linewidth=2, 
            markersize=6, 
            label='Welfare % Change (25yr)', 
            zorder=3, 
            alpha=0.8
        )
    
    # Plot UMPY % change (5yr - solid)
    if not df_5yr.empty and df_5yr['umpy_percent_change'].notna().any():
        ...
        ax.plot(
            ...
            linewidth=2, 
            markersize=6, 
            label='UMPY % Change (5yr)', 
            zorder=3
        )
    
    # Plot UMPY % change (25yr - dashed)
    ...
    ax.set_title("Welfare and UMPY % Change vs Bond Amount", fontsize=16, fontweight='bold')
